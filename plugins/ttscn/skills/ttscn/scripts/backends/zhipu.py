"""Zhipu GLM-TTS backend — REST, reuses ZHIPUAI_API_KEY."""
...
def synthesize(chunks, config, output_file, output_format="wav"):
    """Synthesize via glm-tts /audio/speech, concat into output_file.

    config keys: key, voice, model, speech_rate
    Non-streaming response is an audio/wav file (24 kHz sample rate).
    ...
    key = config["key"]
    voice = config.get("voice", "tongtong")
    model = config.get("model", "glm-tts")
    ...
    for i, chunk in enumerate(chunks):
        ...
        for attempt in range(1, 4):
            try:
                ...
                if not resp.content:
                    raise RuntimeError("GLM-TTS returned empty audio")
