...
def parse_srt(path):
    ...
    for block in re.split(r'\n\n+', content.strip()):
        ...
        try:
            ...
            skipped += 1
            continue
        entries.append({'index': idx, 'start': m.group(1), 'end': m.group(2),
...
def _print_check_whisper_text(result):
    ...
    if result['fallback']:
        print()
        print('Note: openai-whisper is already installed. You can use it as a fallback:')
...
def _detect_font():
    ...
    for candidate in ('Noto Sans CJK SC', 'Source Han Sans SC',
...
def to_ass(entries, preset='clean', font=None, resolution=(1920, 1080),
           top_lang='zh', style_file=None, reflow=False, fade_ms=150,
           safe_area=0.06):
    ...
    if style_file:
        ...
        style_lines = _build_preset_style_lines(p, font, top_lang, resolution, safe_area)
...
if __name__ == '__main__':
    ...
    p_ass.add_argument('--font', default=None,
    ...
    p_ass.add_argument('--reflow', action='store_true',
                       help='Split each cue into complete-sentence lines (rule 10)')
    p_ass.add_argument('--fade', type=int, default=150,
                       help='Fade in/out duration in ms (0 disables; default: 150)')
    p_ass.add_argument('--safe-area', type=float, default=0.06,
    ...
    # --- to_ass ---
        ...
        try:
            ass_content = to_ass(entries, preset=args.preset, font=args.font,
                                ...
                                style_file=args.style_file, reflow=args.reflow,
                                fade_ms=args.fade, safe_area=args.safe_area)
        except (OSError, ValueError) as e:
            _emit(
                {'ok': False, 'command': 'to_ass',
                 'error': {'code': 'runtime_error', 'message': str(e), 'retryable': False}},
